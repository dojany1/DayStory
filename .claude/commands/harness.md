...
사용자가 자동 적용을 요청하면 명령어 입력을 요구하지 말고 에이전트가 직접 아래 자동 실행을 수행한다.

```bash
npm run harness:auto
```

수동으로 특정 phase만 실행해야 하면 아래 명령을 사용한다.

```bash
npm run harness -- {task-name}
npm run harness -- {task-name} --push
```

scripts/codex-harness.mjs가 자동으로 처리하는 것:

- pending phase 자동 탐색
- Codex CLI와 Claude CLI 중 사용 가능한 에이전트 자동 선택
- dirty worktree를 삭제하지 않고 snapshot commit으로 보존한 뒤 실행
- `feat-{task-name}` 브랜치 생성/checkout
- 가드레일 주입 — AGENTS.md + CLAUDE.md + docs/*.md 내용을 매 step 프롬프트에 포함
...
주의: scripts/codex-harness.mjs가 시작한 step 세션 안에서는 `npm run harness:auto`를 다시 실행하지 않는다. 이유: 재귀 실행 방지.

### F. 대화형 자동 실행

사용자가 "알아서 해줘", "계속해", "적용해", "구현해", "진행해", "fix it", "continue"처럼 자연어로 작업 진행을 요청하면 먼저 pending harness phase가 있는지 확인한다.

- pending phase가 있으면 `npm run harness:auto`를 직접 실행한다.
- pending phase가 없으면 사용자의 요청을 일반 작업으로 바로 진행한다.
- 순수 질문, 설명, 리뷰, 계획만 요청한 경우에는 사용자가 적용이나 실행을 요청하지 않는 한 자동 실행하지 않는다.
- 자동화를 준비하면서 사용자 작업을 삭제하거나 reset하지 않는다. dirty worktree는 snapshot commit으로 보존한다.
