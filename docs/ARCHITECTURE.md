...
│   │   │   stories.js · mystories.js · bookmarks.js · notifications.js · widget.js
│   │   ├── utils/
│   │   │   date.js · sanitize.js
│   │   └── config/         # 환경 상수
│   └── css/
...
- **사진 업로드**: 파일 선택 → MIME/크기 사전 검사(이미지 + 10MB 이하) → CropperJS 모달 → browser-image-compression 으로 WebP 변환 → Storage 업로드 → URL 을 폼에 주입. 업로드 중에는 저장 버튼 비활성화.
