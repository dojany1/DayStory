...
## 튜토리얼
- 앱 내 튜토리얼은 제거된 상태다. 설정 화면에도 “튜토리얼 다시 보기” 행을 두지 않는다.
- 새 안내 UI가 필요해지기 전까지 `.tutorial-*`, `.tour-*`, `data-tour-target` 기반 코드를 다시 추가하지 않는다.
- 홈 화면 첫 카드 진입은 편지/튜토리얼 연출 없이 `.card-drop-enter` 로 위에서 아래로 짧게 내려오는 모션만 사용한다.
...
- 홈 카드 첫 진입: `card-drop-enter` (opacity + translateY -32px → 0)
