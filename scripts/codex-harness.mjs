...
function buildInstructions(phaseName, stepN, stepContent, meta) {
  ...
  const lines = [
    ...
    `2. docs/SESSION_LOG.md — 최근 작업 이력 (맨 아래부터 역순으로 읽어 현재 상태 파악)`,
    `3. docs/ARCHITECTURE.md — 모듈 구조 및 의존성`,
    `4. docs/CODE_MAP.md — 파일별 역할 지도`,
    ...
    ``,
    `3. docs/SESSION_LOG.md 맨 아래에 세션 항목을 추가한다 (CLAUDE.md 규칙 준수).`,
    ``,
